...
const specialLists = require('./utils/item-managers')
...
class WorkerInfo {
  ...
  constructor(worker,appMaster) {
    ...
    this.address = ''
	if ( appMaster ) {
		appMaster.emit('cworker',worker,this)
	}
  }
  // --connectTimeout--------------------------------------- 
  ...
  connectTimeout() {
    ...
    if ( worker !== undefined ) {
      if ( !(worker.isConnected()) ) {
        ...
        //
        //if ( diff > MAX_CONNECT_FAIL_TIME  )  console.log('log',`DISCONNECT TIMEOUT ${this.worker_key}`)
        //
...
class ClusterManager extends EventEmitter {
  ...
  optionDefaults(opt) {
	    ...
	    if ( this.opt.logger === undefined ) {
			this.opt.logger = require('./logger')
	    }
  }
  ...
  doFork() {
    ...
    if ( Object.keys(tTracked).length < nW ) {
      ...
      //
      tTracked[worker.id] = new WorkerInfo(worker,this)
      //
